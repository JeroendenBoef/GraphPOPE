...
def main():
    ...
    #num_anchor_nodes=32
        wandb_logger = WandbLogger(name=f'{args.sampling_method}-{num_anchor_nodes}',project='GraphPOPE-Flickr')
        seed_everything(42)
        path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'data', 'Flickr')
        datamodule = Flickr(path, batch_size=args.batch_size, num_anchor_nodes=num_anchor_nodes)
        model = SaintGCN(datamodule.num_features, datamodule.num_classes)
        checkpoint_callback = ModelCheckpoint(monitor='val_acc', save_top_k=1, dirpath=osp.join(osp.dirname(osp.realpath(__file__)), '..', 'data', 'flickr_checkpoint'))
        #trainer = Trainer(gpus=[0], max_epochs=10, callbacks=[checkpoint_callback])

        # Uncomment to train on multiple GPUs:
        trainer = Trainer(gpus=2, accelerator='ddp', max_epochs=50,
