...
class Visitor(gt.BFSVisitor):
    ...
    def examine_vertex(self, u):
        ...
        if u == self.target:
            #print(f'node {u} found {self.dist[u]} hops away from {self.source_node}')
...
def shortest_path_length_gt(G, source_nodes):
    ...
    for source_node in source_nodes:
        for target_node in G.get_vertices():
            ...
            if len(dists_dict.keys()) % 200 == 0:
                save_path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'save', f'products_distances_{len(dists_dict.keys())}.pkl')
...
def shortest_path_gt(G, num_workers=6):
    """
    ...
    try:
        ...
        pool.join()
        print('finished!')
        save_path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'save', f'products_distances_full.pkl')
        with gzip.open(save_path, "wb") as f:
            pickled = pickle.dumps(dists_dict)
            optimized_pickle = pickletools.optimize(pickled)
            f.write(optimized_pickle)
