    ...
    for source_node in source_nodes:
        for target_node in G.get_vertices():
            ...
            if len(dists_dict.keys()) % 200 == 0:
                save_path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'save', f'products_distances_1_mil_{len(dists_dict.keys())}.pkl')
...
def shortest_path_gt(G, num_workers=6):
    ...
    try:
        ...
        pool.join()
        print('finished!')
        save_path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'save', f'products_distances_1mil_full.pkl')
        with gzip.open(save_path, "wb") as f:
            pickled = pickle.dumps(dists_dict)
            optimized_pickle = pickletools.optimize(pickled)
            f.write(optimized_pickle)
