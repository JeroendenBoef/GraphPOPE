...
class Flickr(LightningDataModule):
    def __init__(self, data_dir: str, sweep_hyperparams,
                 in_memory: bool = False):
        super().__init__()
        self.data_dir = data_dir
        self.batch_size = sweep_hyperparams.batch_size
        self.num_anchor_nodes = sweep_hyperparams.num_anchor_nodes
...
class SAGE(LightningModule):
    def __init__(self, in_channels: int, out_channels: int,
                 hidden_channels: int, sweep_hyperparams, dropout: float = 0.5):
        super().__init__()
        # self.save_hyperparameters()
        self.dropout = dropout
        self.num_layers = sweep_hyperparams.num_layers
...
def main(config):
    ...
    datamodule = Flickr(path, config)
    
    # Lightning model
    model = SAGE(in_channels=datamodule.num_features, out_channels=datamodule.num_classes, hidden_channels=args.hidden_layer_size, sweep_hyperparams=config)
    
    # Wandb logger
    wandb_logger = WandbLogger(name=f'cc_bsize_{args.batch_size}',project='sweep_cc')
    ...
    # Trainer
    trainer = Trainer(gpus=[0], accelerator='ddp', max_epochs=args.epochs, logger=wandb_logger, checkpoint_callback=True, 
